...
jest.mock('../QuantitySelector', () => (props: {
  ...
  <div data-testid="quantity-selector">
    <button onClick={() => props.handleQuantityChange(props.quantity - 1)}>-</button>
...
describe('CartItem Component', () => {
  ...
  it('should render product image, name, and price', () => {
    ...
    expect(screen.getByAltText(mockProduct.nome)).toHaveAttribute('src', mockProduct.imagem);
    expect(screen.getByText(mockProduct.nome)).toBeInTheDocument();
    expect(screen.getByText(`R$ ${mockProduct.preço.toFixed(2)}`)).toBeInTheDocument();
  });
  ...
  it('should call handleQuantityChange with the correct value when quantity buttons are clicked', () => {
    ...
    expect(mockHandleQuantityChange).toHaveBeenCalledWith(mockProduct.quantity + 1);
